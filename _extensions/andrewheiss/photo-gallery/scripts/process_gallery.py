...
def generate_thumbnail(src: Path, thumb_path: Path, max_width: int, max_height: int, quality: int):
    ...
    with Image.open(src) as img:
        ...
        # JPEG has no alpha/palette support: flatten transparency onto white, else coerce to RGB
        if thumb.mode in ("RGBA", "LA", "P"):
            thumb = thumb.convert("RGBA")
            background = Image.new("RGBA", thumb.size, (255, 255, 255, 255))
            thumb = Image.alpha_composite(background, thumb).convert("RGB")
        elif thumb.mode != "RGB":
